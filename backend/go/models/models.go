...
type OpenAIJobParsingResponse struct {
	...
	MinYearsExperience        *int     `json:"MinYearsExperience" jsonschema:"nullable,minimum=0,maximum=25" jsonschema_description:"Minimum years of professional software engineering experience explicitly required by the source. If the source states a number or range (for example, '3+ years' or '2-4 years'), return the minimum/lower bound. Return 0 only when the source explicitly states that no prior professional experience is required. Otherwise return null. Never infer years from the job title or seniority labels such as Entry-level, Junior, Mid-level, Senior, Staff, Principal, Lead, Director, New Grad, or Intern."`
