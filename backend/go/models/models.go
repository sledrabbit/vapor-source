...
type OpenAIJobParsingResponse struct {
	...
	MinYearsExperience        *int     `json:"MinYearsExperience" jsonschema:"nullable,minimum=0,maximum=25" jsonschema_description:"Lowest minimum years of professional software engineering experience across all valid qualification paths in the source. For an explicit required number or range, return its lower bound. Return 0 when at least one valid path requires no prior professional experience, including complete requirements that accept education, coursework, an internship, or new-graduate qualifications without an additional professional-experience requirement. Never return 0 for a role identified as Senior or Sr., Staff, Principal, or Director; return null if such a role has no explicit quantifiable minimum. Seniority may rule out 0 but must never supply a positive fallback number. Return null when the minimum cannot otherwise be determined, including missing or visibly truncated qualifications and unquantified mandatory experience. Do not use preferred experience or infer positive years solely from a title."`
